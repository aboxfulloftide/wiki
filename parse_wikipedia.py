...
class WikipediaParser:
    ...
    def _search_in_text(self, text, search_term, case_sensitive=False, use_regex=False):
        """
        Search for term in text, with optional regex support.
        Returns (found, count) tuple.
        
        Args:
            text: Text to search in
            search_term: Term or pattern to search for
        ...
        if use_regex:
            ...
            try:
                ...
                print(f"Warning: Invalid regex pattern '{search_term}': {e}")
                print("Falling back to literal string search")
    ...
    def search_index(self, search_term, case_sensitive=False, use_regex=False):
        ...
        try:
            for line in tqdm(file_handle, desc="Scanning index", unit=" lines"):
                ...
                if len(parts) == 3:
                    ...
                    # Use string search or regex to handle special characters
                    found, _ = self._search_in_text(page_title, search_term, case_sensitive, use_regex)
    ...
    def extract_pages_streaming(self, target_page_ids, search_term, case_sensitive, output_file, checkpoint=None, use_regex=False):
        """
        ...
        if not checkpoint:
            ...
            output_handle.write(f"Regex mode: {use_regex}\n")
            output_handle.write(f"{'='*60}\n")
            ...
            for page in dump:
                ...
                if current_page_id in target_ids_set and current_page_id not in found_ids:
                    ...
                    for revision in page:
                        ...
                        # Count occurrences using string search or regex
                        found_in_text, text_count = self._search_in_text(text, search_term, case_sensitive, use_regex)
                        found_in_title, title_count = self._search_in_text(title, search_term, case_sensitive, use_regex)
    ...
    def search_with_index(self, search_term, case_sensitive=False, search_content=False, output_file=None, use_regex=False):
        """Search using index file for efficiency"""
        # First, search the index for matching titles
        index_matches = self.search_index(search_term, case_sensitive, use_regex)
        ...
        found_ids, pages_processed, last_page_id, completed = self.extract_pages_streaming(target_page_ids, search_term, case_sensitive, output_file, checkpoint, use_regex)
    ...
    def parse_compressed_xml(self, search_term, case_sensitive=False, output_file=None, use_regex=False):
...
python parse_wikipedia.py --search "^\d+s(?: (?:BC|AD))?$" --regex --use-index --output decades.txt
    ...
    parser.add_argument(
        '--regex',
        action='store_true',
        help='Treat search term as regex pattern (default: False)'
    )
